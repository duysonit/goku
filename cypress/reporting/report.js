 ...
 */
 ...
 function addScreenshots() {
   ...
   */
   ...
   const readdirRecursive = (location) => readdirPreserveRelativePath(location).reduce((result, currentValue) => (fs.statSync(currentValue).isDirectory() ? result.concat(readdirRecursive(currentValue)) : result.concat(currentValue)), []);
 ...
 function addSnapshots() {
   ...
   // Extract feature list from snapshots list
   ...
   featuresList.forEach((feature) => {
     snapshots.forEach((snapshot) => {
       ...
       myScenarios.forEach((myScenario) => {
         const myStep = myScenario.steps.find((step) => step.result.status === "failed");
         if (!myStep) {
           return;
         }
 ...
 function generateReport() {
   if (!fs.existsSync(cucumberJsonDir)) {
     ...
     report.generate({
       ...
       pageTitle: "Autonomous Report",
       reportName: `Autonomous Report - ${new Date().toLocaleString()}`,
       hideMetadata: true,
     });
   }
 }
